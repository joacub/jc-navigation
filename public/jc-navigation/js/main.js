...
//			e.preventDefault();
//			$(this).closest('.content').find('input[type="checkbox"]:visible').each(function() {
//				
//				if($(this).is(':checked')) {
//					var clone = $('#jc-navigation-db-template').clone();
//					var item = $('<li/>', {});
//					item.append(clone.show());
//					$( "#menu-to-edit" ).append(item);
//					$(this).attr('checked', false);
//					var data = jQuery.parseJSON($(this).val());
//					clone.find('h4 span').text(data.title);
//				}
//				
//			});
//			
		...
		var api = wpNavMenu = {
			...
			jQueryExtensions : function() {
				...
				$.fn.extend({
					...
					addSelectedToMenu : function(processMethod) {
						...
						return this.each(function() {
							var t = $(this), menuItems = {},
								checkboxes = t.find('.active .categorychecklist li input:checked');
							...
							$(checkboxes).each(function(){
								var t = $(this),
									listItemData = jQuery.parseJSON(t.val());
								
								if ( this.className && -1 != this.className.indexOf('add-to-top') )
									processMethod = api.addMenuItemToTop;
								menuItems[listItemData.collector + '_' + listItemData.id] = listItemData;
							});
					...
					setItemData : function() { // Can take a type, such as 'menu-item', or an id.
						this.find('input').each(function() {
							var t = $(this);
							if(t.attr('type') == 'checkbox') {
								
								if(t.attr('original-value') == 'true') {
									t.val(t.attr('checked', 'checked'));
								} else {
									t.val(t.removeAttr('checked'));
								}
								
							} else {
								t.val(t.attr('original-value'));
							}
...
//				api.menuList.hideAdvancedMenuItemFields();
			...
			addItemToMenu : function(menuItem, processMethod, callback) {
				...
				$.post( site_url + '/es/admin/JcNavigation/add-menu-item', params, function(menuMarkup) {

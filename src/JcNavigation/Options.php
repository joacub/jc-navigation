...
class Options extends AbstractOptions
{
    ...
    protected function setMatcher($options)
    {
        if (!is_array($options)) {
            $this->report->addError(sprintf(
                '[\'JcNavigation\'][\'profiler\'][\'matcher\'] must be an array, %s given.',
                gettype($options)
            ));
    ...
    protected function setCollectors($options)
    {
        if (!is_array($options)) {
            $this->report->addError(sprintf(
                '[\'JcNavigation\'][\'profiler\'][\'collectors\'] must be an array, %s given.',
                gettype($options)
            ));
    ...
    public function setToolbar(array $options)
    {
        ...
        if (isset($options['position'])) {
            if ($options['position'] !== 'bottom' && $options['position'] !== 'top') {
                $this->report->addError(sprintf(
                    '[\'JcNavigation\'][\'toolbar\'][\'position\'] must be "top" or "bottom", %s given.',
                    $options['position']
                ));
            } else {
        ...
        if (isset($options['entries'])) {
            if (is_array($options['entries'])) {
                ...
                $this->report->addError(sprintf(
                    '[\'JcNavigation\'][\'toolbar\'][\'entries\'] must be an array, %s given.',
